...
export default function Upload() {
  ...
  const loadMaterials = async () => {
    ...
    try {
      const [materialsData, examsData, flashcardsData] = await Promise.all([
        api.getMaterials({ user_id: user.id }),
        api.getExams({ user_id: user.id }),
        api.getFlashcards({ user_id: user.id })
      ]);

      setMaterials(materialsData);
      setUpcomingExams(examsData.filter((e: any) => e.score === null));
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/upload?user_id=${user.id}`, {
        method: 'POST',
        body: formData
      });
